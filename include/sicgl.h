...
// a height or width of zero will make the grid un-writeable
// keeping this simple interface makes it very easy to write drawing code
// other functionalities, such as window management, should be handled externally
// for instance:
// WAIT - how DO you do this? you could of course create an interface which maps grid units into
// some other global coordinate system... is this fast? is this an extra lookup for each pixel? each
// drawing interface? can you even take advantage of bounds checking if you are then translating to 
// some other coordinate system? (maybe you CAN because you know that the resulting coordinates are
// all bound within the original window...)
...
typedef void (*hline_fn_t)(void* arg, color_t color, uext_t u0, uext_t v, uext_t u1);
typedef void (*vline_fn_t)(void* arg, color_t color, uext_t u, uext_t v0, uext_t v1);
typedef void (*region_fn_t)(void* arg, color_t color, uext_t u0, uext_t v0, uext_t u1, uext_t v1);
