...
void run_test_case(location_test_case_t test_case) {
  ...
  if (generic) {
    ...
    generic_region(intfc, test_color, u0, v0, u1, v1);
    ...
    TEST_ASSERT_NOT_NULL_MESSAGE(intfc, "could not allocate interface object");
    specific_region(intfc, test_color, u0, v0, u1, v1);
...
void test_region(void) {
  ...
  for (uint32_t count = 0; count < regions; count++) {
    ...
    generic_region(fast_intfc, &pixel, u0, v0, u1, v1);
    generic_region(naive_intfc, &pixel, u0, v0, u1, v1);
    specific_region(specfic_intfc, &pixel, u0, v0, u1, v1);
  }
