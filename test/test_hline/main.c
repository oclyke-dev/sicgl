...
void run_test_case(location_test_case_t test_case) {
  ...
  TEST_ASSERT_EQUAL_MEMORY_MESSAGE(reference, bytes->memory, bytes_length_bytes(*bytes), case_name);
  release_bytes(bytes);
}
...
void test_hline(void) {
  ...
  // store images
  png_to_file(fast_bm, "test_hline_fast.png");
  png_to_file(naive_bm, "test_hline_naive.png");
  png_to_file(spec_bm, "test_hline_specific.png");
