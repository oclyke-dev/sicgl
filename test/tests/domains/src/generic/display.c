...
void test_generic_display_circle(void) {
  ...
  if (TEST_PROTECT()) {
    ...
    // draw a rectangle
    int color = truecolor_random_color();
		int ret = simultaneous_generic_display_circle(reference, interface, &color, width/2, height/2, 20);
    ...
    TEST_ASSERT_NOT_NULL_MESSAGE(img, "could not create img png");
    TEST_ASSERT_EQUAL_INT(
        0, png_to_file(ref, TEST_OUTPUT_DIR "/generic_display_circle_ref.png"));
    TEST_ASSERT_EQUAL_INT(
        0, png_to_file(img, TEST_OUTPUT_DIR "/generic_display_circle_img.png"));
    TEST_ASSERT_EQUAL_INT_MESSAGE(0, ret, "failed to draw rectangle");
